      ...
      try {
        ...
        if (!res.ok) {
          ...
          if (paymentGuard?.requiresUpgrade || paymentGuard?.requiresRenewal) {
            ...
            router.replace({
              pathname: "/paywall",
              params: { source: "lesson-status", mode },
            } as any);
            return;
          }
        ...
        setStatus((prev) => ({
          ...
          videoCompleted:
            data.videoProgress?.isCompleted ?? prev?.videoCompleted ?? false,
          quizCompleted:
            data.quizResult?.isCompleted ?? prev?.quizCompleted ?? false,
        ...
        Alert.alert(
          "Lỗi",
          error?.message ?? "Không thể tải trạng thái bài học.",
        );
      } finally {
        setIsLoading(false);
      }
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (paymentGuard?.requiresUpgrade || paymentGuard?.requiresRenewal) {
          ...
          router.push({
            pathname: "/paywall",
            params: { source: "lesson-video", mode },
          } as any);
          return;
        }
        const text = await res.text().catch(() => "");
      ...
      Alert.alert(
        "Lỗi",
        error?.message ?? "Không thể đánh dấu đã xem xong video.",
      );
    } finally {
    ...
    return (
      ...
      >
        ...
        <Text style={[styles.lockedText, { color: colors.secondaryText }]}>
          Bài học này đang bị khóa. Hãy hoàn thành bài trước đó.
        </Text>
      </SafeAreaView>
    );
  }
        ...
        <View style={[styles.videoCard, { backgroundColor: colors.cardBg }]}>
          ...
          <TouchableOpacity
            style={[
              styles.videoButton,
              {
                backgroundColor: status.videoCompleted
                  ? "#10B981"
                  : colors.tint,
              },
            ]}
          ...
          >
            {isMarkingVideo ? (
              ...
              <Text style={styles.videoButtonText}>
                {status.videoCompleted
                  ? "Đã đánh dấu xem xong video"
                  : "Đánh dấu xem xong video"}
              </Text>
            )}
          </TouchableOpacity>
        </View>
          ...
          <View style={styles.quizHeader}>
            ...
            {status.quizCompleted && (
              <Text
                style={[
                  styles.quizBadge,
                  { color: "#10B981", backgroundColor: colors.qaGreenBg },
                ]}
              >
                Đã hoàn thành
              </Text>
